...
class VulnerableHouseholdsScreen extends StatelessWidget {
  const VulnerableHouseholdsScreen({
    super.key,
    required this.api,
    required this.data,
        ...
        builder: (context, setSheetState) => Padding(
          ...
          child: ListView(
            ...
            children: [
              const PageTitle(
                'Thêm hộ ưu tiên',
                'Ghi nhận hộ cần hỗ trợ đặc biệt khi sơ tán/cứu hộ',
              ),
  ...
  Future<void> updatePriority(
    ...
    await api.updateVulnerableHousehold(valueOf(household, 'id'), {
      'priority_level': priority,
    });
    await onChanged();
  }
  ...
  Widget build(BuildContext context) {
    return AppList(
      children: [
        const PageTitle(
          'Hộ dễ tổn thương',
          'Ưu tiên người già, trẻ em, khuyết tật và bệnh nền',
        ),
        ElevatedButton.icon(
          onPressed: () => showHouseholdForm(context),
          icon: const Icon(Icons.add_home),
          label: const Text('Thêm hộ ưu tiên'),
        ),
                ...
                children: [
                  Row(
                    children: [
                      ...
                      BadgePill(
                        label: 'Ưu tiên ${valueOf(h, 'priority_level')}',
                        bg: Palette.warning.withValues(alpha: .14),
                        fg: Palette.warning,
                      ),
                    ],
                  ),
                  ...
                  Wrap(
                    ...
                    children: [
                      for (final priority in const [
                        ...
                        TextButton(
                          onPressed: () => updatePriority(h, priority),
                          child: Text(levelLabel(priority)),
                        ),
                    ],
                  ),
                ],
