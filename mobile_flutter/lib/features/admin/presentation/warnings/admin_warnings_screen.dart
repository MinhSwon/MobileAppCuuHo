...
class _AdminWarningsState extends State<AdminWarnings> {
  ...
  Future<void> updateWarningStatus(
  ...
  Future<void> deleteWarning(Map<String, dynamic> warning) async {
    final ok = await confirmAction(
      context,
      'Xóa cảnh báo?',
      'Cảnh báo "${valueOf(warning, 'title')}" sẽ bị xóa khỏi hệ thống.',
    );
    if (!ok) return;
  ...
  Future<void> editWarning(Map<String, dynamic> warning) async {
    final titleCtrl = TextEditingController(text: valueOf(warning, 'title'));
    final contentCtrl = TextEditingController(
      text: valueOf(warning, 'content'),
    );
    String editLevel = valueOf(warning, 'level', fallback: 'HIGH');
        ...
        builder: (ctx, setSheet) => Padding(
          ...
          child: ListView(
            ...
            children: [
              const PageTitle(
                'Chỉnh sửa cảnh báo',
                'Cập nhật nội dung cảnh báo đang phát',
              ),
              ...
              Wrap(
                spacing: 8,
                children: ['LOW', 'MEDIUM', 'HIGH', 'EMERGENCY']
                    .map(
                      (l) => ChoiceChip(
                        label: Text(levelLabel(l)),
                        selected: editLevel == l,
                        onSelected: (_) => setSheet(() => editLevel = l),
                      ),
                    )
                    .toList(),
              ),
              ...
              ElevatedButton.icon(
                onPressed: () async {
                  ...
                  await widget.api
                      .updateWarningContent(valueOf(warning, 'id'), {
        ...
        CardBox(
          child: Column(
            children: [
              ...
              const SizedBox(height: 12),
              ElevatedButton.icon(
                onPressed: createWarning,
                icon: const Icon(Icons.campaign),
                label: const Text('Phát cảnh báo'),
              ),
            ],
          ),
        ),
        ...widget.data.warnings.map(
          (w) => WarningCard(
            warning: w,
            actions: [
              TextButton.icon(
                onPressed: () => editWarning(w),
    ...
    return AppList(
      children: [
        ...
        if (warnings.isEmpty)
          const EmptyCard(
            icon: Icons.campaign,
            message: 'Không có cảnh báo đang hoạt động',
          )
        else
