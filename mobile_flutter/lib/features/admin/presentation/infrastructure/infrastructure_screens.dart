...
class AdminSafeZonesScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) =>
      SafeZonesScreen(data: data, api: api, onChanged: onChanged);
}

class RescueRoutesScreen extends StatelessWidget {
  const RescueRoutesScreen({
    super.key,
    required this.api,
    required this.data,
    required this.onChanged,
        ...
        builder: (context, setSheetState) => Padding(
          ...
          child: ListView(
            ...
            children: [
              const PageTitle(
                'Thêm tuyến cứu hộ',
                'Cập nhật tuyến tiếp cận nạn nhân và điểm sơ tán',
              ),
              ...
              TextField(
                controller: from,
                decoration: const InputDecoration(labelText: 'Điểm đầu'),
              ),
              const SizedBox(height: 10),
              TextField(
                controller: to,
                decoration: const InputDecoration(labelText: 'Điểm cuối'),
              ),
              const SizedBox(height: 10),
              TextField(
                controller: distance,
              ...
              DropdownButtonFormField<String>(
                ...
                items: const ['OPEN', 'CAUTION', 'BLOCKED', 'CLOSED']
                    .map(
                      (s) => DropdownMenuItem(
                        value: s,
  ...
  Future<void> deleteRoute(
    ...
    final ok = await confirmAction(
      context,
      'Xóa tuyến đường?',
      'Tuyến "${valueOf(route, 'name')}" sẽ bị xóa.',
    );
    if (!ok) return;
  ...
  Widget build(BuildContext context) {
    return AppList(
      children: [
        const PageTitle(
          'Tuyến đường cứu hộ',
          'Tình trạng tuyến tiếp cận nạn nhân và điểm sơ tán',
        ),
        ElevatedButton.icon(
          onPressed: () => showRouteForm(context),
          icon: const Icon(Icons.add_road),
                  ...
                  Wrap(
                    ...
                    children: [
                      for (final status in const [
                        ...
                        TextButton(
                          onPressed: () => updateRouteStatus(r, status),
  ...
  Widget build(BuildContext context) {
    return AppList(
      children: [
        const PageTitle(
          'Đập/Hồ chứa',
          'Theo dõi mực nước và trạng thái vận hành',
        ),
        if (data.dams.isEmpty)
          const EmptyCard(
            icon: Icons.water_drop,
            message: 'Chưa có dữ liệu hồ chứa',
          )
        else
          ...data.dams.map(
            (d) => CardBox(
              child: Column(
                ...
                children: [
                  ...
                  InfoRow(
                    'Dung tích',
                    '${valueOf(d, 'current_volume_percent')}%',
                  ),
                  Text(
                    valueOf(d, 'note'),
                    style: const TextStyle(color: Palette.secondary),
  ...
  Widget build(BuildContext context) {
    return AppList(
      children: [
        const PageTitle(
          'Báo cáo thiệt hại',
          'Tổng hợp báo cáo nhà cửa, đường sá, nông nghiệp',
        ),
        if (data.damageReports.isEmpty)
          const EmptyCard(
            icon: Icons.description,
            message: 'Chưa có báo cáo thiệt hại',
          )
        else
          ...data.damageReports.map(
            (r) => CardBox(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
...
class SafeZonesScreen extends StatelessWidget {
        ...
        builder: (context, setSheetState) => Padding(
          ...
          child: ListView(
            ...
            children: [
              const PageTitle(
                'Thêm điểm sơ tán',
                'Cập nhật nơi tiếp nhận người dân an toàn',
              ),
              const SizedBox(height: 12),
              TextField(
                controller: name,
              ...
              DropdownButtonFormField<String>(
                initialValue: status,
                decoration: const InputDecoration(labelText: 'Trạng thái'),
                items: const ['AVAILABLE', 'FULL', 'CLOSED', 'INACTIVE']
                    .map(
                      (s) => DropdownMenuItem(
                        value: s,
                        child: Text(statusLabel(s)),
  ...
  Future<void> updateSafeZoneStatus(
  ...
  Future<void> deleteSafeZone(
    ...
    final ok = await confirmAction(
      context,
      'Xóa điểm sơ tán?',
      'Điểm "${valueOf(zone, 'name')}" sẽ bị xóa.',
    );
    if (!ok) return;
  ...
  Widget build(BuildContext context) {
    return AppList(
      children: [
        const PageTitle(
          'Điểm sơ tán',
          'Sức chứa, liên hệ và tình trạng còn chỗ',
        ),
        if (canEdit)
          ElevatedButton.icon(
            onPressed: () => showSafeZoneForm(context),
            icon: const Icon(Icons.add),
          ...
          (s) => CardBox(
            child: Column(
              ...
              children: [
                ...
                InfoRow(
                  'Sức chứa',
                  '${valueOf(s, 'current_people')}/${valueOf(s, 'capacity')} người',
                ),
                InfoRow(
                  'Liên hệ',
                  '${valueOf(s, 'contact_person')} · ${valueOf(s, 'contact_phone')}',
                ),
                if (canEdit) ...[
                  const SizedBox(height: 8),
                  Wrap(
                    ...
                    children: [
                      for (final status in const [
                        ...
                        TextButton(
                          onPressed: () => updateSafeZoneStatus(s, status),
                          child: Text(statusLabel(status)),
                        ),
                      TextButton.icon(
                        style: TextButton.styleFrom(
...
class TeamsScreen extends StatelessWidget {
        ...
        builder: (context, setSheetState) => Padding(
          ...
          child: ListView(
            ...
            children: [
              const PageTitle(
                'Thêm đội cứu hộ',
                'Tạo nhanh lực lượng điều phối trên di động',
              ),
              const SizedBox(height: 12),
              TextField(
                controller: name,
              ...
              TextField(
                controller: leader,
                decoration: const InputDecoration(labelText: 'Trưởng đội'),
              ),
              const SizedBox(height: 10),
              TextField(
                controller: phone,
              ...
              DropdownButtonFormField<String>(
                initialValue: status,
                decoration: const InputDecoration(labelText: 'Trạng thái'),
                items: const ['AVAILABLE', 'BUSY', 'OFFLINE', 'INACTIVE']
                    .map(
                      (s) => DropdownMenuItem(
                        value: s,
                        child: Text(statusLabel(s)),
  ...
  Future<void> deleteTeam(
    ...
    final ok = await confirmAction(
      context,
      'Xóa đội cứu hộ?',
      'Đội "${valueOf(team, 'team_name')}" sẽ bị xóa.',
    );
    if (!ok) return;
  ...
  Widget build(BuildContext context) {
    return AppList(
      children: [
        ...
        ElevatedButton.icon(
          onPressed: () => showTeamForm(context),
          icon: const Icon(Icons.add),
          label: const Text('Thêm đội cứu hộ'),
        ),
        ...data.teams.map(
          (t) => CardBox(
            child: Column(
              ...
              children: [
                ...
                InfoRow(
                  'Phương tiện',
                  valueOf(t, 'vehicle_type', fallback: '-'),
                ),
                const SizedBox(height: 8),
                Wrap(
                  spacing: 6,
                  ...
                  children: [
                    for (final status in const ['AVAILABLE', 'BUSY', 'OFFLINE'])
                      TextButton(
                        onPressed: () => updateTeamStatus(t, status),
                        child: Text(statusLabel(status)),
                      ),
                    TextButton.icon(
                      style: TextButton.styleFrom(
