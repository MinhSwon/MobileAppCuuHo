...
class MissionScreen extends StatelessWidget {
  const MissionScreen({super.key, required this.api, required this.user, required this.data, required this.onChanged});
  final ApiClient api;
  ...
  Widget build(BuildContext context) {
    final isRescue = ['RESCUE_LEADER', 'RESCUE_MEMBER'].contains(valueOf(user, 'role'));
    final team = isRescue ? currentRescueTeam(user, data) : null;
    final missions = isRescue && team != null
        ? data.missions.where((m) => valueOf(m, 'rescue_team_id') == valueOf(team, 'id')).toList()
        : data.missions;
    return AppList(
      children: [
        const PageTitle('Nhiệm vụ cứu hộ', 'Cập nhật trạng thái theo tiến độ thực tế'),
        if (missions.isEmpty)
